    ...
    */
    private void updateCommMan(int stateIndex) {
        if (stateIndex == 1) { // we have logged in reads in accounts file
            ...
            commMan = new LoggedInState(temporaryTransactions, validAccounts, masterTransactions, keyboard);
        }
        if (stateIndex == 2)
            commMan = new AtmState(temporaryTransactions, validAccounts, masterTransactions, keyboard);
        if (stateIndex == 3)
            commMan = new AgentState(temporaryTransactions, validAccounts, masterTransactions, keyboard);
